...
function App() {
  ...
  useEffect(() => {
    const cargarAlertas = async () => {
      try {
        const res = await fetch('https://vecinos-virtuales-backend-3.onrender.com/api/alertas');
  ...
  const enviarAlerta = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        e.target.reset();
        // Recargar alertas
        const nuevas = await (await fetch('https://vecinos-virtuales-backend-3.onrender.com/api/alertas')).json();
